<?php
$sezonica='season > 0 AND season < 99';
...
$comparepasss = mysql_query("SELECT club, supporter, natcoach, nt_country, lang, `level` FROM users WHERE password ='$koda' AND userid ='$userid' LIMIT 1") or die(mysql_error());
...
$levelll = mysql_result($comparepasss,0,"level");
}
...
[<a href="playerstats.php?playerid=<?=$playerid?>"><?=$langmark1287?></a>] [<a href="playerstats.php?playerid=<?=$playerid?>&show=career"><?=$langmark1420?></a>] 
[<?=$langmark1421?>]

<?php
//stevilo tekem
$sttekem1 = mysql_query("SELECT COUNT(*) AS kokot1 FROM nt_statistics WHERE $sezonica AND type < 10 AND player=$playerid");
$tekem11 = @mysql_result($sttekem1, 0, "kokot1"); if (!$tekem11 OR !is_numeric($tekem11)) {$tekem11=0;}
$sttekem2 = mysql_query("SELECT COUNT(*) AS kokot2 FROM nt_statistics WHERE $sezonica AND type > 9 AND player=$playerid");
$tekem22 = @mysql_result($sttekem2, 0, "kokot2"); if (!$tekem22 OR !is_numeric($tekem22)) {$tekem22=0;}
$skupaj_tekem = $tekem11 + $tekem22;
if ($skupaj_tekem==0) {die("<ul><li>$langmark998</li></ul></td></tr></table>");}

//minutaza
$gametime = mysql_query("SELECT SUM(gametime) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_gametime) = mysql_fetch_row($gametime);
//za dve
$za_dve_zadel = mysql_query("SELECT SUM(two_scored) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_dve_zadel) = mysql_fetch_row($za_dve_zadel);
//za ena
$za_ena_zadel = mysql_query("SELECT SUM(one_scored) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_ena_zadel) = mysql_fetch_row($za_ena_zadel);
//za tri
$za_tri_zadel = mysql_query("SELECT SUM(three_scored) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_tri_zadel) = mysql_fetch_row($za_tri_zadel);
//za dve
$za_dve = mysql_query("SELECT SUM(two_total) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_dve) = mysql_fetch_row($za_dve);
//za ena
$za_ena = mysql_query("SELECT SUM(one_total) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_ena) = mysql_fetch_row($za_ena);
//za tri
$za_tri = mysql_query("SELECT SUM(three_total) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_za_tri) = mysql_fetch_row($za_tri);
//def skoki
$def_skoki = mysql_query("SELECT SUM(def_rebounds) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($obr_skoki) = mysql_fetch_row($def_skoki);
//off skoki
$off_skoki = mysql_query("SELECT SUM(off_rebounds) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($nap_skoki) = mysql_fetch_row($off_skoki);
//blokade
$blocks = mysql_query("SELECT SUM(blocks) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_blocks) = mysql_fetch_row($blocks);
//podaje
$assists = mysql_query("SELECT SUM(assists) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_assists) = mysql_fetch_row($assists);
//ukradene
$steals = mysql_query("SELECT SUM(steals) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_steals) = mysql_fetch_row($steals);
//osebne
$fouls = mysql_query("SELECT SUM(fouls) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_fouls) = mysql_fetch_row($fouls);
//napake
$turnovers = mysql_query("SELECT SUM(turnovers) FROM nt_statistics WHERE $sezonica AND player=$playerid") or die(mysql_error());
list($sum_turnovers) = mysql_fetch_row($turnovers);
//rating
...
<table>
<?php if($skupaj_tekem==1){?><tr><td><b><?="1 ",$langmark1551?></b><?php }?>
<?php if($skupaj_tekem==2){?><tr><td><b><?="2 ",$langmark1552?></b><?php }?>
<?php
if($skupaj_tekem > 2 AND $tekem11 > 0 AND $tekem22 > 0) {echo "<tr><td><b>",$skupaj_tekem," ",$langmark1553," (",$tekem11,"+",$tekem22,")</b>";}
elseif ($skupaj_tekem > 2){echo "<tr><td><b>",$skupaj_tekem," ",$langmark1553,"</b>";}
?>
