...
if (mysql_num_rows($result) < 1) {header("location: players.php"); die();}
...
if ($playerid=='579851') {$country='China';}
...
//bookmark
if ($action=='bookmark') {
$already = mysql_query("SELECT * FROM bookmarks WHERE b_type =1 AND team ='$trueclub' AND b_link ='$id' LIMIT 1") or die(mysql_error());
$skupay = mysql_query("SELECT * FROM bookmarks WHERE team=$trueclub AND b_type=1") or die(mysql_error());
if (mysql_num_rows($already) > 0) {echo "<table cellpadding=\"9\" cellspacing=\"9\" width=\"100%\"><tr bgcolor=\"#ffffff\"><td class=\"border\">",$langmark407," ",$langmark094," <a href=\"bookmarks.php\">",$langmark095,"</a>.</td></tr></table>";}
elseif (mysql_num_rows($skupay) > 400) {echo "<table cellpadding=\"9\" cellspacing=\"9\" width=\"100%\"><tr bgcolor=\"#ffffff\"><td class=\"border\">You have reached the limit of 400 bookmarked players.<br/>You need to remove some players from your bookmarks to be able to add new ones.</td></tr></table>";
}
else
{
$ime = $name." ".$surname;
$zapis = mysql_query("INSERT INTO bookmarks VALUES ('', $trueclub, '$ime', $id, 1, 0);") or die(mysql_error());
...
echo "<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" width=\"100%\"><tr><td><b>",$shirtnum," ",$name," ",$surname," (",$id,")</b>";
...
if ($is_supporter==1) {echo "&nbsp;<a href=\" transferhistory.php?playerid=",$playerid,"&action=bookmark\"><img src=\"img/bookmark.jpg\" alt=\"",$langmark410,"\" title=\"",$langmark410,"\" border=\"0\"></a>";}
...
if ($is_supporter==1){ ?><table cellpadding="0" cellspacing="0" width="56%"><tr><td><img src="faces/template.php?kepal=<?=$face?>&teling=<?=$ears?>&mat=<?=$eyes?>&mulut=<?=$mouth?>&hidung=<?=$nose?>&kumis=<?=$mustage?>&rambut=<?=$hair?>" border="1" alt="<?=$hesay?>" title="<?=$hesay?>"></td><td align="left"><table cellpadding="0" cellspacing="1" width="99%"><?php } else { ?><table cellpadding="0" cellspacing="0" width="100%"><?php }?>
...
<tr><td><b><?=$langmark411,":</b> ",number_format($value, 0, ',', '.')?> &euro;
<?php
...
<tr><td width="163"><b><?=$langmark426?>:</b> <?=$handling_dspl?></td>
...
<br/><h2><?=$langmark466?></h2><br/>
<?php
$result654 = mysql_query("SELECT * FROM transfers WHERE trstatus =0 AND playerid='$playerid' ORDER BY transferid DESC") or die(mysql_error());
?>
<table border="0" cellspacing="0" cellpadding="1" width="100%">
<tr>
<td width="65"><b><?=$langmark349?></b></td>
<td width="163"><b><?=$langmark1065?></b></td>
<td width="163"><b><?=$langmark1066?></b></td>
<td align="right" width="74"><b><?=$langmark924?></b></td>
<td align="right" width="59"><b><?=$langmark141?></b></td>
</tr></table><hr/>
<?php
while ($f =mysql_fetch_array($result654)) {
$tranerid=$f[transferid];
$playerclub=$f[playerclub];
$sellingid=$f[sellingid];
$timeofsale=$f[timeofsale];
$price1=$f[price];
$currentbid=$f[currentbid];
$bidingteam=$f[bidingteam];
$bidingname=$f[bidingname];
$valuethen=$f[EV];
if ($currentbid>999 AND $sellingid<>$bidingteam) {$totc=$totc+$currentbid;}
if ($currentbid==0) {$currentbid=$price1; $cura=0;} else {$cura=1;}
if ($cura==1 AND $sellingid<>$bidingteam AND $sellingid<>0) {$kola='PaleGreen';} else {$kola='white';}
if ($valuethen==0) {$valuethen='&nbsp;';} else {$valuethen = number_format($valuethen, 0, ',', '.'); $grg=44;}
if ($tranerid < 4002076 AND $grg==44) {$valuethen = "<span title=\"Old value\" style=\"color:gray\">".$valuethen."</span>";}

$pricedisplay = number_format($currentbid, 0, ',', '.');
if ($currentbid==1){$pricedisplay=$langmark1067;}
...
<table border="0" cellspacing="0" cellpadding="1" width="100%">
<tr bgcolor="<?=$kola?>"><td width="65"><?=$disdate?></td>
<td width="163">
<?php
if($sellingid==0) {echo $playerclub,"</td>";}else{?><a href="club.php?clubid=<?=$sellingid?>"><?=$playerclub,"</a></td>";}?>
<td width="163"><?php if($bidingname =='Free Transfer'){echo $langmark1068,"</td>";}elseif($cura==0){echo "<font color=\"darkred\">NO BID</font></td>";}else{?><a href="club.php?clubid=<?=$bidingteam?>"><?=$bidingname,"</a></td>";}?>
<td align="right" width="74"><?php if ($sellingid<>$bidingteam OR ($sellingid==$bidingteam && $sellingid==$userid)) {echo $pricedisplay;} else {echo "-";}?></td>
<td align="right" width="59"><?php if ($sellingid<>$bidingteam OR ($sellingid==$bidingteam && $sellingid==$userid)) {echo $valuethen;} else {echo "-";}?>
<?php if ($gm ==3 AND $price1 > $currentbid AND $sellingid<>$bidingteam AND $currentbid >0) {?><tr bgcolor="LightSalmon"><td colspan="5"><b>GM INFO ONLY: above transfer was adjusted from <?=number_format($price1, 0, ',', '.')?> to <?=$pricedisplay?>.</b></td></tr><?php }?>
</table>
<?php
}
if ($totc > 999) {echo "<hr/>Total transfers worth: ".number_format($totc, 0, ',', '.')." &euro;<br/>";}
?>
<br/>[<a href="player.php?playerid=<?=$playerid?>" class="greenhilite"><<</a>]
</td>
</tr>
</table>
</div>
