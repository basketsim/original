<?
	////////////////////////////////////////////////////////////////////////////////
	// This script is used for CBPP national team match details						
	...
	if(strlen($matchid))
	{
		...
		if($r && @mysql_num_rows($r))
		{
			...
			$qn	= "SELECT country AS name FROM countries WHERE countryid = '{$a['home_id']}'";
			...
			$qn	= "SELECT country AS name FROM countries WHERE countryid = '{$a['away_id']}'";
			$rn	= @mysql_query($qn, $connect);
			if($rn && @mysql_num_rows($rn))
			{
				$an				= mysql_fetch_array($rn);
...
$xml	= "<?xml version=\"1.0\"?>
<BasketSimData>
	<FileName>nationalteammatchdetails.php</FileName> 
	<Version>1.0</Version> 
