...
<body>
	...
	<div id="header">
		<div id="admin-header">
			...
			<ul>
				<li><a class="logout" href="<?php echo ThumbsUp_Admin::url('action=logout', FALSE) ?>"><button>Logout</button></a></li>
			...
			<?php foreach ($items as $id => $item) { ?>
				<tr>
					...
					<td class="action left">
						<div class="edit">
							<button class="edit">Edit</button> <a class="delete" href="#" title="Delete">&#9747;</a>
						</div>
						<div class="save">
	...
	<div id="footer">
		...
		<div class="credits">
			...
			©2009–<?php echo date('Y') ?> <a href="http://codecanyon.net/user/GeertDD?ref=GeertDD">Geert De Deckere</a>
		</div>
	</div>
	...
	$(document).ready(function() {
		...
		$(':submit.save').not(':disabled').click(function() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$this.closest('div').hide().siblings('div').show();
					$row.find(':input').not($this).not('.edit').attr('disabled', 'disabled');
		...
		$('.cancel').click(function() {
			...
			$row.find(':input').not('.edit').attr('disabled', 'disabled');
