...
class ThumbsUp_Admin {
	...
	public function action_login()
	{
		...
		if (isset($_POST['username']) AND isset($_POST['password']))
		{
			...
			{
				// Show an error if no admin user is defined
				if (empty($users) OR ! is_array($users))
				{
					trigger_error('You need to setup an admin user first in thumbsup/config.php in order to be able to login.');
				}
			}

			// Convert all passwords to sha1 if needed
			foreach ($users as $_username => $_password)
			{
				// A sha1 string are 40 hexadecimal characters
				if ( ! preg_match('~^[0-9a-f]{40}$~D', (string) $_password))
				{
					$users[$_username] = sha1($_password);
				}
			}
