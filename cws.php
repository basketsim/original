...
$krogpokala = mysql_query("SELECT MAX(lid_round) FROM matches WHERE season=$sseason AND type=7") or die(mysql_error());
...
<?php
$cupmatches = mysql_query("SELECT `home_id`, `home_name`, `away_id`, `away_name`, `home_score`, `time` FROM `matches` WHERE `type` =7 AND `season` =$sseason AND `lid_round` =$round ORDER BY `time` ASC, matchid ASC");
...
if ($mat > 1) {
$hwi=0; $awi=0;
$ddmatche = mysql_query("SELECT home_id, away_id, home_score, away_score FROM matches WHERE type=7 AND season=$sseason AND (home_id=$home_id AND away_id=$away_id OR home_id=$away_id AND away_id=$home_id)") or die(mysql_error());
...
<?php
$lura=0;
$dodmatche = mysql_query("SELECT matchid, home_id, away_id, crowd1, home_score, away_score, time FROM matches WHERE type=7 AND season=$sseason AND (home_id=$home_id AND away_id=$away_id OR home_id=$away_id AND away_id=$home_id) ORDER BY matchid DESC") or die(mysql_error());
...
<!--<td align="left"><img src="http://i165.photobucket.com/albums/u71/kaloian40/Logos%20and%20kits/heat_zps984a7ed3.png" border="0" width="140"></td><td align="center"><b>VS.</b></td><td align="right"><img src="http://oi44.tinypic.com/2nhet1y.jpg" border="0" width="140"></td>--></tr></table>
<?php
}
if (($grlo==44) OR ($sseason==3 AND $round==5) OR ($sseason >3 AND $sseason <9 AND $round==6)) {
//MVP
$hahaha = mysql_query("SELECT player, name, surname, teamname FROM top_players, players WHERE player=players.id AND type=7 AND season='$sseason'") or die(mysql_error());
...
echo "<br/>Cup Winners Series MVP award: <a href=\"player.php?playerid=",$idMVP,"\">",$namMVP," ",$surMVP,"</a> (",stripslashes($teamMVP),")";
} elseif($sseason==$default_season) {echo "<br/><i>Cup Winners Series MVP will be awarded after the final is played.";} else {echo "<br/>Cup Winners Series MVP: <i>retired player</i>";}
}
?>

</td><td class="border" width="36%">

<big>Cup Winners Series</big><br/>
...
Cup Winners Series <?=$langmark1407?> <a href="gamerules.php?action=international"><?=$langmark1408?></a>.<br/>

<br/><h2>Outright odds</h2><br/>
<?php
$supp3 = $_GET["supp"];
if ($supp3==10 AND $suppo==1) {
$bzook = mysql_query("SELECT userid, name, odds FROM ekipe, teams, users WHERE club=ekipa and teamid=club and competition=2");
...
if ($gemn==2 OR $gemn==3) {$oddsLI=$oddsLI/1.29;} elseif ($gemn==4) {$oddsLI=$oddsLI/1.7;} elseif ($mat>15) {$oddsLI=$oddsLI/0.45;}
...
$bzook = mysql_query("SELECT userid, name, odds FROM ekipe, teams, users WHERE club=ekipa and teamid=club and competition=2 LIMIT 5");
...
if ($suppo==1 AND $supp3<>10 AND $glj==5) {echo "[<a href=\"cws.php?supp=10\" title=\"",$langmark231,"\" class=\"greenhilite\">+</a>]<br/>";}
if ($suppo==1 AND $supp3==10) {echo "[<a href=\"javascript: history.go(-1)\" title=\"",$langmark409,"\" class=\"greenhilite\">-</a>]<br/>";}

$offset_result = mysql_query("SELECT FLOOR(RAND() * COUNT(*)) AS `offset` FROM `players`, `ekipe` WHERE `players`.`club` = `ekipe`.`ekipa` AND `players`.`statement` != '' AND ekipe.competition=2");
$offset_row = mysql_fetch_object($offset_result);
$offset = $offset_row->offset;
$puma = mysql_query("SELECT `players`.`id`, `players`.`name`, `players`.`surname`, `players`.`statement` FROM `players`, `ekipe` WHERE `players`.`club` = `ekipe`.`ekipa` AND `players`.`statement` != '' AND ekipe.competition=2 LIMIT $offset, 1");
...
<tr><td><br/></td></tr>
<?php
$zmagovalci = mysql_query("SELECT `h_season`, `h_userid`, `h_teamid`, `h_teamname`, `h_country` FROM `history` WHERE `h_type`=7 AND `won` =1 ORDER BY `h_season` DESC") or die(mysql_error());
...
if (mysql_num_rows($preveritev)) {echo "<tr><td><a href=\"cws.php?season=",$h_season,"&round=7\" title=\"",$langmark502," ",$h_season,"\">",$dizp,"</a>&nbsp;&nbsp;<img src=\"img/Flags/",$h_country,".png\" border=\"1\" alt=\"",$h_country,"\" title=\"",$h_country,"\">&nbsp;&nbsp;<a href=\"club.php?clubid=",$h_userid,"\">",stripslashes($h_teamname),"</a></td></tr>";} else {echo "<tr><td><a href=\"cws.php?season=",$h_season,"&round=7\" title=\"",$langmark502," ",$h_season,"\">",$dizp,"</a>&nbsp;&nbsp;<img src=\"img/Flags/",$h_country,".png\" border=\"1\" alt=\"",$h_country,"\" title=\"",$h_country,"\">&nbsp;&nbsp;",stripslashes($h_teamname),"</td></tr>";}
}?>
</table>
</td>
</tr>
